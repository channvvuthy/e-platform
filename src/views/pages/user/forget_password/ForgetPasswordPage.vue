    ...
    <div :style="{ height: `${windowHeight}px` }" class="grid grid-cols-2 gap-10">
        ...
        <div class="px-10 flex items-center flex-col justify-center">
            <form class="w-full pr-10" @submit.prevent="onVerify">
                ...
                <PrimaryButton :isLoading="isLoading" btnTitle="Verify" />
                <SpaceHeight height="30" />
                <div>
...
import ForgetPasswordHeader from "./partials/ForgetPasswordHeader.vue";
import ForgetPasswordContent from "./partials/ForgetPasswordContent.vue";
...
const onVerify = async (e) => {
    const { phone } = extractFormData(e, ['phone',]);
    const validationFields = makeRequiredOnly([{ 'field': 'phone', 'val': phone, }]);
    const isErrors = validateFields(validationFields);
    const deviceInfo = await window.electron.getDeviceInfo();

    if (!isErrors.length) {
        store.dispatch('user/verifyAccountOwner', { phone, ...deviceInfo })
            .then(({ data }) => {
                storeUserData(data);
